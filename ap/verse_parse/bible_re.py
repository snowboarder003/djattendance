...
('Exodus', 'Exod', 'Exo(?:\.|dus)', [22, 25, 22, 31, 23, 30, 25, 32, 35, 29, 10, 51, 22, 31, 27, 36, 16, 27, 25, 26, 36, 31, 33, 18, 40, 37, 21, 43, 46, 38, 18, 35, 23, 35, 35, 38, 29, 31, 43, 38]),
...
('I Chronicles', '1Chr', '(?:1|I)(?:\s*)Chron(?:\.|icles)', [54, 55, 24, 43, 26, 81, 40, 40, 44, 14, 47, 40, 14, 17, 29, 43, 27, 17, 19, 8, 30, 19, 32, 31, 31, 32, 34, 21, 30]),
('II Chronicles', '2Chr', '(?:2|II)(?:\s*)Chron(?:\.|icles)', [17, 18, 17, 22, 14, 42, 22, 18, 31, 19, 23, 16, 22, 15, 19, 14, 19, 34, 11, 37, 20, 12, 21, 27, 28, 23, 9, 27, 36, 27, 21, 33, 25, 33, 27, 23]),
...
('I Thessalonians', '1Thess', '(?:1|I)(?:\s*)Thes(?:\.|salonians)', [10, 20, 13, 18, 28]),
('II Thessalonians', '2Thess', '(?:2|II)(?:\s*)Thes(?:\.|salonians)', [12, 17, 18]),
...
('James', 'Jas', 'Jam(?:\.|es)', [27, 26, 18, 17, 20]),
...
scripture_re = re.compile(
    r'(?:(?<=\W)(?P<BookTitle>%s)(?=\W))' \
     '(?:\s*(?P<ChapterNumber>\d{1,3})' \
     '(?:(?:\s*:\s*(?P<VerseNumber>\d{1,3}))?' \
     '(?:\s*-\s*' \
     '(?P<EndChapterNumber>\d{1,3}(?=:\d{1,3}))?' \
     '(?::)?' \
     '(?P<EndVerseNumber>\d{1,3})?' \
     ')?(?P<MoreVerseNumbers>(?:,\s*\d{1,3}(?:\s*-\s*\d{1,3})?)*)?)?' \
     ')?' \
     '|' \
     '(?P<HeadlessChapter>\d{1,3})' \
     '(?::(?P<HeadlessVerse>\d{1,3}))' \
     '(?:-(?P<HeadlessEndChapter>\d{1,3}(?=:\d{1,3}))?' \
     '(?::)?' \
     '(?P<HeadlessEndVerse>\d{1,3})?)?' \
     '(?P<HeadlessMoreVerseNumbers>(?:,\s*\d{1,3}(?:\s*-\s*\d{1,3})?)+)?' \
     '|' \
     '(?:(?<=\W)v{1,2}\.\s*(?P<LonelyVerse>\d{1,3})' \
     '(?P<MoreLonelyVerses>(?:,\s*\d{1,3}(?:\s*-\s*\d{1,3})?)+)?)' \
     '' % (book_re_string,))
